...
class LspCompletionWindow(
    ...
    private fun subscribeEvents() {
        eventManager.subscribeAlways(EditorKeyEvent::class.java) { event ->
            if (isShowing && event.eventType == EditorKeyEvent.Type.DOWN) {
                when (event.keyCode) {
                    KeyEvent.KEYCODE_DPAD_UP -> { moveSelection(-1); event.markAsConsumed() }
                    KeyEvent.KEYCODE_DPAD_DOWN -> { moveSelection(1); event.markAsConsumed() }
                    KeyEvent.KEYCODE_TAB, KeyEvent.KEYCODE_ENTER -> {
                        applySelection(currentSelectedIndex)
                        event.markAsConsumed()
                    }
                    KeyEvent.KEYCODE_ESCAPE -> { dismiss(); event.markAsConsumed() }
                }
            }
        }
    ...
    private fun updateWindowLocation() {
        ...
        val windowY = if (spaceBelow < height && charY > height) {
            charY - height - margin
        } else {
    ...
    fun applySelection(index: Int) {
        ...
        item.additionalTextEdits?.let { edits ->
            val sortedEdits = edits.sortedWith(compareBy({ -it.range.start.line }, { -it.range.start.character }))
