...
class DefaultLspCompletionLayout : CompletionLayout {
    ...
    override fun createView(inflater: LayoutInflater): View {
        ...
        // 详细文档容器
        docContainer = LinearLayout(context).apply {
            orientation = LinearLayout.VERTICAL
            layoutParams = LinearLayout.LayoutParams(0, ViewGroup.LayoutParams.MATCH_PARENT, 0.8f)
            visibility = View.GONE
            setPadding((8 * dpUnit).toInt(), (8 * dpUnit).toInt(), (8 * dpUnit).toInt(), (8 * dpUnit).toInt())
        }

        docProgressBar = ProgressBar(context, null, android.R.attr.progressBarStyleSmall).apply {
            visibility = View.GONE
        ...
        docScrollView = ScrollView(context).apply {
            layoutParams = LinearLayout.LayoutParams(ViewGroup.LayoutParams.MATCH_PARENT, 0, 1f)
            isVerticalScrollBarEnabled = false
        }

        docTextView = TextView(context).apply {
        ...
        root.addView(View(context).apply { 
            layoutParams = LinearLayout.LayoutParams((1 * dpUnit).toInt(), ViewGroup.LayoutParams.MATCH_PARENT)
    ...
    override fun showDocumentation(item: CompletionItem) {
        val doc = item.documentation ?: run {
        ...
        val markdownText = if (doc.isLeft) {
        ...
        renderJob = window.launchRender {
            docTextView.text = SimpleMarkdownRenderer.renderAsync(
    ...
    private inner class LspCompletionAdapter : BaseAdapter() {
        ...
        override fun getView(position: Int, convertView: View?, parent: ViewGroup): View {
            ...
            view.setBackgroundColor(if (position == selectedIndex) selectedItemBgColor else Color.TRANSPARENT)
            ...
            val isDeprecated = item.tags?.contains(CompletionItemTag.Deprecated) == true || item.deprecated == true
            // 注：此处 item.label 可能在外部（FuzzyScorer）被替换为 SpannableString 以显示匹配高亮
            val labelText = if (item.label is Spannable) item.label as Spannable else SpannableString(item.label)
            if (isDeprecated) {
                labelText.setSpan(StrikethroughSpan(), 0, labelText.length, Spannable.SPAN_EXCLUSIVE_EXCLUSIVE)
            }
        ...
        private fun createItemView(context: Context): LinearLayout {
            ...
            return LinearLayout(context).apply {
                ...
                addView(ImageView(context).apply {
                    layoutParams = LinearLayout.LayoutParams((16 * dpUnit).toInt(), (16 * dpUnit).toInt()).apply {
