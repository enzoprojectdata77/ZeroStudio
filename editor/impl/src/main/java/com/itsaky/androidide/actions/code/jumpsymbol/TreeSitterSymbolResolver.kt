...
object TreeSitterSymbolResolver {
    ...
    fun parseSymbols(context: Context, file: File, code: String): List<SymbolInfo> {
        ...
        val languageImpl = TreeSitterLanguageProvider.forType(extension, context)
            ?: return emptyList()
        ...
        return parser.use { p ->
            ...
            p.parseString(null, content)?.use { tree ->
                ...
                tree.rootNode?.let { root ->
                    traverseTree(root, symbols, code, extension)
                }
                symbols
            } ?: emptyList()
    ...
    private fun traverseTree(node: TSNode, list: MutableList<SymbolInfo>, sourceCode: String, lang: String) {
        // 在访问子节点之前，先处理当前节点
        // 这确保了在当前函数作用域内，`node` 是有效的
        val symbol = when (lang) {
            "java" -> processJavaNode(node, sourceCode)
            "kt", "kts" -> processKotlinNode(node, sourceCode)
            else -> null
        }
    ...
    private fun processJavaNode(node: TSNode, source: String): SymbolInfo? {
        ...
        return when (type) {
            ...
            "field_declaration" -> {
                ...
                findChildByType(node, "variable_declarator")?.let { 
                    getNodeText(it.getChildByFieldName("name"), source) 
                }?.let { name ->
                     return SymbolInfo(name, typeText, startLine, SymbolType.FIELD)
                }
    ...
    private fun getNodeText(node: TSNode?, source: String): String? {
        ...
        return try {
            ...
            if (start < 0 || end > source.length || start >= end) null
            else source.substring(start, end)
