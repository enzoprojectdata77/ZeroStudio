...
open class PhpLanguage(
    ...
    companion object {
        ...
        @JvmField
        val FACTORY = Factory { PhpLanguage(it, TSLanguagePhp.getInstance(), TS_TYPE_PHP) }

        // 纯 PHP 代码解析工厂 (常用于代码片段注入，不需要 <?php 标签)
        @JvmField
        val PHP_ONLY_FACTORY = Factory { PhpLanguage(it, TSLanguagePhp.getPhpOnlyInstance(), TS_TYPE_PHP_ONLY) }
    }
