 ...
 *
 * This class serves as a bridge between AndroidIDE's environment and the sora-editor's LSP implementation.
 * It combines the responsibilities of defining server metadata, providing a connection mechanism,
 * and handling server-to-client events.
 *
 * ## Work-flow Diagram
 * [LspManager] -> [Get Server for File] -> [BaseLspServer instance]
 *     |
 *     v
 * [BaseLspConnector] -> [server.getConnectionFactory()] -> [Creates StreamConnectionProvider] -> [Starts Server]
 *     |
 *     v
...
abstract class BaseLspServer : LanguageServerDefinition(), EventHandler.EventListener {
    ...
    /**
     * Defines the installation logic for the server. This could involve extracting assets,
     * running scripts, or downloading binaries.
     *
     * @param context The application context.
     */
    ...
    /**
     * Creates the connection provider that sora-editor's LSP client will use to communicate
