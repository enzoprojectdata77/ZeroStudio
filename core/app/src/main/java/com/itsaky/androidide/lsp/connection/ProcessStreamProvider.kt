...
/**
 * A [StreamConnectionProvider] that establishes a connection to a Language Server
 * by launching it as a local process using [ProcessBuilder]. This class is essential for
 * interacting with LSP servers that are executed as command-line tools.
 *
 * ## Work-flow Diagram
 * [start()] -> Creates [ProcessBuilder] with command & environment -> [Process.start()] -> Provides [InputStream] & [OutputStream] from the process.
 *
...
class ProcessStreamProvider(
    ...
    override val inputStream: InputStream
        get() = process?.inputStream ?: throw IOException("Process is not active or has already exited.")
