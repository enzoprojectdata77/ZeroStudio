 ...
 *
 * <p>
 * 定义了分析 Gradle 项目依赖关系、提取仓库配置以及检查版本更新的标准行为规范。
 * 实现类应当负责具体的 DSL 解析（Groovy/Kotlin）和 Version Catalog (TOML) 处理，
 * 屏蔽底层的词法分析与 AST 遍历细节。
 * </p>
 */
interface ProjectAnalyzer {
     ...
     *
     * <p>
     * 扫描项目的构建脚本（根目录及子模块的 build.gradle/kts, settings.gradle/kts），
     * 解析 <code>repositories { ... }</code> 和 <code>pluginManagement { ... }</code> 块中的 Maven 仓库定义。
     * </p>
     *
     ...
     *
     * <p>
     * 通过并发网络请求，在提供的仓库列表中查询每个依赖的 <code>maven-metadata.xml</code>，
     * 并对比版本号以确定是否存在更新。
     * </p>
     *
