...
object TomlDiagnostics {
    ...
    fun compute(content: String): List<Diagnostic> {
        ...
        while (true) {
            ...
            if (tokenType == TomlElementTypes.EQ) {
                ...
                if (text.contains('\n') && expectingValue) {
                    ...
                    diagnostics.add(Diagnostic(range, "Expected value after '='", DiagnosticSeverity.Error, "TOML"))
                    expectingValue = false
                }
            }
        ...
        if (expectingValue) {
             ...
             diagnostics.add(Diagnostic(range, "Unexpected end of file, expected value", DiagnosticSeverity.Error, "TOML"))
        }
    ...
    private fun addDiagnostic(
        ...
        list.add(Diagnostic(Range(startPos, endPos), "$msg: '${content.substring(start, end)}'", DiagnosticSeverity.Error, "TOML"))
    }
