...
/**
 * A utility object that centralizes high-level actions related to the Language Server Protocol.
 * It acts as a bridge between UI components (like context menus) and the underlying [BaseLspConnector],
 * simplifying complex LSP interactions into single method calls.
 *
...
object LspActions {
    ...
    /**
     * Executes the "rename symbol" action. It requests the rename operation from the LSP server
     * and applies the resulting [WorkspaceEdit] to all affected documents.
     *
            ...
            runCatching {
                ...
                workspaceEdit.documentChanges?.forEach { either ->
                    if (either.isLeft) {
                        ...
                        LOG.warn("Resource operations (rename/delete file) not yet supported in AndroidIDE LSP.")
                    }
                }
                
                LOG.info("Rename operation to '$newName' completed.")
            }.onFailure { 
                LOG.error("Rename symbol action failed.", it) 
            }
        }
    }
    ...
    /**
     * Corrects file URIs returned by the LSP server to match the local filesystem paths
     * within the AndroidIDE sandbox.
     *
     ...
     * @param connector The active [BaseLspConnector].
     * @param onJump A callback function that receives the target file, line, and column for navigation.
     */
    fun goToDefinition(
        scope: CoroutineScope,
        connector: BaseLspConnector,
        ...
        scope.launch(Dispatchers.Default) {
            runCatching {
                ...
                if (locations.isNotEmpty()) {
                    ...
                    withContext(Dispatchers.Main) {
                        onJump(file, start.line, start.character)
                    }
                ...
                LOG.error("Go to definition action failed.", it)
            }
        }
    }
}