...
class KotlinAstAnalyzer : ScriptAnalyzer {
    ...
    private fun extractDependency(callNode: TSNode, configName: String, text: String, file: File, deps: MutableList<ScopedDependencyInfo>) {
    ...
    private fun extractRepository(callNode: TSNode, funcName: String, text: String, file: File, repos: MutableList<ScopedRepositoryInfo>) {
        if (funcName == "google") {
            ...
            if (lambdaNode != null) {
                ...
                val matcher = Regex("""url\s*=\s*uri\s*\(\s*["']([^"']+)["']\s*\)""").find(lambdaText) 
