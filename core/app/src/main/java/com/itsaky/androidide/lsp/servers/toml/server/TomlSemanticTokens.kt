...
class TomlSemanticTokens(private val content: String) {
    ...
    fun compute(): List<Int> {
        ...
        while (true) {
            ...
            val tokenIndex = when (tokenType) {
                // Keywords & Headers
                TomlElementTypes.TABLE_HEADER,
                TomlElementTypes.L_BRACKET, TomlElementTypes.R_BRACKET,
                TomlElementTypes.L_CURLY, TomlElementTypes.R_CURLY,
                TomlElementTypes.INLINE_TABLE,
                TomlElementTypes.ARRAY, TomlElementTypes.ARRAY_TABLE -> 0 // keyword
