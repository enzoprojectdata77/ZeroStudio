...
/**
 * A central registry and manager for all available Language Server Protocol (LSP) servers in AndroidIDE.
 *
 * It handles:
...
object LspManager {
    ...
    /**
     * List of built-in servers (static).
     * Mutable state list to allow UI updates if needed, though usually fixed after bootstrap.
     */
    private val _builtinServers = mutableStateListOf<BaseLspServer>()

    /**
     * List of user-defined external servers.
     * These are persisted to disk.
     */
    val externalServers = mutableStateListOf<BaseLspServer>()

    /**
     * Configuration map (ID -> Config).
     * Stores enabled state and user-defined file extensions.
     */
    val serverConfigs = mutableStateMapOf<String, LspServerConfig>()

    /**
     * A combined, read-only list of ALL available servers from all sources.
     * Used by the UI to display the full list.
     *
     * Priority/Order: Built-in -> External -> Extensions
     * 
     * This is a derived state, so it will automatically update whenever
     * _builtinServers, externalServers, or ExtensionLspRegistry.servers changes.
     */
    ...
    /**
     * Initializes the manager. Must be called before accessing any other methods.
     * Usually called in Application.onCreate or Main Activity.
     */
    ...
    /**
     * Registers built-in servers (called by [LspBootstrap]).
     * Existing servers with the same ID will be skipped to prevent duplicates.
     */
    fun registerServers(servers: List<BaseLspServer>) {
        ...
        if (addedCount > 0) {
            LOG.info("Registered $addedCount new built-in LSP servers. Total built-in: ${_builtinServers.size}")
        } else {
    ...
    /**
     * Finds a specific server instance by its ID.
     * This is crucial for configuring specific servers (e.g. setting Lua library path)
     * before connection.
     */
    ...
    fun getEffectiveExtensions(server: BaseLspServer): List<String> {
        ...
        return (server.supportedExtensions + userExts)
            .map { it.lowercase() }
            .distinct()
            .sorted()
    }
    ...
    /**
     * Adds or updates a user-defined external server.
    ...
    /**
     * Returns all currently registered server definitions.
     * This is primarily for non-reactive access if needed.
     */
    ...
    private fun createServerFromData(data: ExternalServerData): BaseLspServer {
        return when (data.type) {
            ...
            "process" -> {
                ...
                val fullCommand = if (data.args.isNotEmpty()) {
    ...
    private fun externalServerToData(server: BaseLspServer): ExternalServerData? {
        return when (server) {
            is ExternalSocketServer -> ExternalServerData(
                id = server.id,
            ...
            is ExternalProcessServer -> ExternalServerData(
                id = server.id,
                name = server.languageName,
                type = "process",
                extensions = server.supportedExtensions,
