...
object LspStatusMonitor {
    ...
    enum class ServerStatus {
        STOPPED, STARTING, INITIALIZING, RUNNING, ERROR
    }
    ...
    fun updateStatus(serverId: String, name: String, status: ServerStatus, error: String? = null) {
        ...
        val newState = current?.copy(status = status, error = error) 
    ...
    fun updateTraffic(serverId: String, sentDelta: Long, recvDelta: Long) {
        ...
        serverStates[serverId] = current.copy(
    ...
    fun addLog(type: LogType, serverId: String, summary: String, detail: String? = null) {
        synchronized(logs) {
            ...
            logs.add(LogEntry(logIdCounter.getAndIncrement(), System.currentTimeMillis(), type, serverId, summary, detail))
        }
    }
