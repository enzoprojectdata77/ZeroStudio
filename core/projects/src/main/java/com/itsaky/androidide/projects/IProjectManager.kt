...
interface IProjectManager {
  ...
  companion object {
    ...
    fun getInstance(): IProjectManager {
      return projectManager ?: ServiceLoader.load(IProjectManager::class.java).findFirstOrThrow().also {
          projectManager = it
      }
    }
  }

  val projectDirPath: String get() = projectDir.path
  val projectDir: File
  ...
  suspend fun setupProject(project: IProject = Lookup.getDefault().lookup(BuildService.KEY_PROJECT_PROXY)!!)
