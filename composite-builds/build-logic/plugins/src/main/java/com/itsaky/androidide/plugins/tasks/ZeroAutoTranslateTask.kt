 ...
 *
 * 功能：自动读取指定的 Android strings.xml 资源，使用多种翻译引擎多线程并发翻译到多国语言。
 * 支持多文件、多引擎调度、精准保持原始XML注释、空行、行号与格式不被破坏。
 * 
 * 使用示例:
 * tasks.register<ZeroAutoTranslateTask>("translateStrings") {
 *     // 支持多个文件，使用逗号隔开
 *     sourceXmlPaths = "core/resources/src/main/res/values/strings.xml, feature/src/main/res/values/strings.xml"
 *     
...
open class ZeroAutoTranslateTask : DefaultTask() {
    ...
    @get:Input
    var sourceXmlPaths: String = ""

    @get:Input
    var targetModules: List<String> = emptyList()

    @get:Input
    var translationEngines: String = "GOOGLE_WEB"
    ...
    @get:Input
    var translationOutputDirName = "StringTranslation"

    @get:Input
    var originalFileBackupDirName = "StringTranslation/backup"
    ...
    fun translate() {
        ...
        val enginesNames = translationEngines.split(",").map { it.trim().uppercase() }.filter { it.isNotEmpty() }
        ...
        try {
            for (filePath in files) {
                val sourceFile = File(filePath).let { if (it.isAbsolute) it else File(projectRoot, filePath) }
    ...
    private fun processLanguage(
        ...
        if (existingResourceFile.exists()) {
            try {
                Files.copy(existingResourceFile.toPath(), File(backupFolder, sourceFileForRef.name).toPath(), StandardCopyOption.REPLACE_EXISTING)
        ...
        if (targetFile.exists()) {
            parseStringsFromXmlText(targetFile.readText(Charsets.UTF_8)).forEach { existingTranslations[it.name] = it.value }
        }
     ...
     * 将翻译内容无损塞回原模板 XML 中
     */
    private fun saveReconstructedXml(templateText: String, translations: Map<String, String>, targetFile: File) {
    ...
    class GoogleWebTranslator : ITranslator {
        ...
        override fun translate(text: String, from: String, to: String): String {
            val url = "https://translate.googleapis.com/translate_a/single?client=gtx&dt=t&sl=$from&tl=$to&q=${URLEncoder.encode(text, "UTF-8")}"
    ...
    class BingWebTranslator : ITranslator {
        ...
        override fun convertLangCode(lang: String): String = when(lang) {
            "auto" -> "auto-detect"
            "zh-CN" -> "zh-Hans"
            "zh-TW" -> "zh-Hant"
            "iw" -> "he"
    ...
    class DeepSeekTranslator(private val apiKey: String) : ITranslator {
        ...
        override fun translate(text: String, from: String, to: String): String {
            ...
            val response = HttpUtils.post("https://api.deepseek.com/chat/completions")
    ...
    class BaiduWebTranslator : ITranslator {
        ...
        override fun init() {
            ...
            val html = HttpUtils.get("https://fanyi.baidu.com/translate")
        ...
        override fun translate(text: String, from: String, to: String): String {
            ...
            val response = HttpUtils.post("https://fanyi.baidu.com/basetrans")
                .header("Cookie", cookie)
                .formData("query", text)
                .formData("from", from)
                .formData("to", to)
    ...
    class BaiduApiTranslator(private val appId: String, private val key: String) : ITranslator {
        ...
        override fun translate(text: String, from: String, to: String): String {
            ...
            val url = "https://fanyi-api.baidu.com/api/trans/vip/translate?q=${URLEncoder.encode(text, "UTF-8")}&from=$from&to=$to&appid=$appId&salt=$salt&sign=$sign"
            ...
            while (F < text.length) {
                ...
                if (128 > p) {
                    ...
                    if (2048 > p) {
                        ...
                        if (55296 == (64512 and p) && F + 1 < text.length && 56320 == (64512 and text[F + 1].code)) {
